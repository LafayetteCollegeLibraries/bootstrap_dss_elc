...
!function ($) {
  ...
  * ============== */
  ...
  LafayetteDssModal.alertError = function($element, html) {
      ...
      .hide()
      .insertAfter($element.parents('.modal-content').children('.modal-header'))
		  ...
		  if($(this).attr('id') != 'islandora-dss-solr-advanced-search-form') {
			  ...
			  $.post($(this).attr('action'), $(this).serialize(), function(data, textStatus) {
				  ...
				  if($alertError.length > 0) {
				      ...
				      var modalHeader = that.$element.parents('.modal-header:first');
			  ...
			  LafayetteDssModal.alertError($(this), '<li>Your Name field is required.</li><li>Your E-Mail Address field is required.</li><li>Subject field is required.</li><li>Message field is required.</li>');
